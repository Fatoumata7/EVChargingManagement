...
class Society:
    ...
    def __init__(self, f_id: int, config: config.SimulationConfig):
        ...
        for k in self.strategy:
            v = self.strategy[k]
            self.strategy[k] = max(0.1, v + np.random.uniform(-v / 2, v / 2))
    ...
    def update_strategy(self):
        ...
        print(f'---> PERF (total_nb_allocated_slot): {perf}')
        best_idx = int(np.argmax(perf))
        self.best_strategy = copy.deepcopy(self.stations[best_idx].strategy)
        print(f'best_station perf: {perf[best_idx]} allocated_slots')
        print(f'best_strategy = {self.best_strategy}')
